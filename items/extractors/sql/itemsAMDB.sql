WITH
  item_status_with_desc AS (
    ...
    FROM AMDB.item_status istat
      LEFT JOIN AMDB.item_status_type itype ON istat.item_status = itype.item_status_type
  ),
...
SELECT
  ...
  mi.freetext,
  mi.year,
  l_p.location_id AS perm_loc_id,
  l_t.location_id AS temp_loc_id,
  it_p.item_type_code AS perm_type_code,
  it_t.item_type_code AS temp_type_code,
  ib.item_barcode,
  ib.barcode_status,
  'ITEM_AMDB' AS schema
FROM AMDB.item i
  ...
  INNER JOIN AMDB.mfhd_item mi ON i.item_id = mi.item_id
  ...
  INNER JOIN AMDB.item_type it_p ON i.item_type_id = it_p.item_type_id
  LEFT JOIN AMDB.item_type it_t ON i.temp_item_type_id <> 0 AND i.temp_item_type_id = it_t.item_type_id
  LEFT JOIN AMDB.item_barcode ib ON ib.barcode_status = '1' AND i.item_id = ib.item_id
ORDER BY i.item_id
;
